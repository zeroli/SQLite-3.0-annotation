...
*/
#define NEXT(A) (*(char**)(((unsigned long long)A)+offset))
...
int (*cmp)();
{
  ...
  char *set[LISTSIZE];
  int i;
  offset = (unsigned long long)next - (unsigned long long)list;
      ...
      case OPT_FDBL:
        ...
        if( *end ){
          if( err ){
            ...
            errline(i,((unsigned long long)end)-(unsigned long long)argv[i],err);
          }
          errcnt++;
        }
        break;
